...
{	
	...
	this.hitTestObject = function(obj)
	{
		if(this.left().x <= obj.right().x && 
		   ...
		   this.bottomRight().y >= obj.left().x &&
		   this.bottomLeft().y >= obj.top().y &&
		   this.bottomLeft().y >= obj.right().x &&
	...
	this.drawDebug = function()
	{
		...
		context.fillRect(this.bottomLeft().x-size/2, this.bottomLeft().y-size, size, size);
		context.fillRect(this.bottomRight().x-size/2 , this.bottomRight().y-size, size, size);
